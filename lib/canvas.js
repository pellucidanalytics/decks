...
_.extend(Canvas.prototype, binder, hasEmitter, /** @lends Canvas.prototype */ {
  ...
  panToElement: function(element, xOffset, yOffset) {
    validate(element, "Canvas#panToElement: element", { isElement: true });
    this.gestureHandler.animateMoveToElement(element, xOffset, yOffset);
  },
  ...
  configureGestures: function() {
    ...
    var canvasDefaultGestureHandlerOptions = this.defaultGestureHandlerOptions;

    var canvasInstanceGestureHandlerOptions = {
      animator: this.animator,
      config: this.config,
      emitter: this.emitter,
      bounds: this.frameBounds,
      movement: {
        scrollContainerElement: this.frame.element
      }
    };
    ...
    var gestureHandlerOptions = _.merge(
      {},
      canvasDefaultGestureHandlerOptions,
      canvasInstanceGestureHandlerOptions,
  ...
  onViewportRenderErased: function(e) {
    var render = e.data;
    if (this.config.debugDrawing) {
      console.log("Canvas#onViewportRenderErased: render removed");
    }
