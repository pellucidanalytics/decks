...
var Swipable = require("./ui").Swipable;
var MouseWheelScrollable = require("./ui").MouseWheelScrollable;
...
Canvas.defaultGestureOptions = {
  mouseWheel: {
    enabled: false,
    horizontal: false,
    vertical: true,
    Component: MouseWheelScrollable
  },
  pan: {
    enabled: false,
    horizontal: false,
...
_.extend(Canvas.prototype, binder, /** @lends Canvas.prototype */ {
  ...
  configureGestures: function() {
    var defaultOptions = Canvas.defaultGestureOptions;
    var layoutOptions = services.layout.getCanvasGestureOptions();

    this.gestureOptions = _.merge({}, defaultOptions, layoutOptions);

    console.log("canvas: configure gestures", this.gestureOptions);
    ...
    _.each(this.gestureOptions, function(config, key) {
      if (this._gestures[key]) {
        this._gestures[key].destroy();
        delete this._gestures[key];
      }

      config.bounds = this.frameBounds;
      config.hammer = this.hammer;

      // If enabled, create a new gesture handler
      if (config.enabled) {
        this._gestures[key] = new config.Component(this.element, config);
      }
    }, this);
  },
