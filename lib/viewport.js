...
function Viewport(options) {
  ...
  this.options = options = _.merge({}, this.defaultOptions, options);
...
_.extend(Viewport.prototype, binder, hasEmitter, /** @lends Viewport.prototype */ {
  ...
  drawRender: function(render) {
    ...
    var renderSlotIndex = parseInt(self.rendersDrawingCount / this.options.itemsPerRenderSlot, 10);
    var slotRenderDelay = this.options.slotRenderDelay;

    // Disable animation if the item falls in later slots
    if (renderSlotIndex >= this.options.maxSlotsWithAnimations ||
