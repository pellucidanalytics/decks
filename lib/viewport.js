...
_.extend(Viewport.prototype, binder, hasEmitter, /** @lends Viewport.prototype */ {
   ...
   * @return {undefined}
   */
  drawItem: function drawItem(item) {
    if (this.config.debugDrawing) {
      console.log("Viewport#drawItem: drawing item", item);
    }
  ...
  drawItems: function drawItems() {
    if (this.config.debugDrawing) {
      console.log("Viewport#drawItems: drawing items");
    }
  ...
  unloadRender: function unloadRender(render) {
    if (this.config.debugLoading) {
      console.log("Viewport#unloadRender: unloading render", render);
    }
  ...
  drawCustomRender: function drawCustomRender(customRender) {
    ...
    if (self.config.debugDrawing) {
      console.log("Viewport#drawCustomRender: drawing custom render (" + self.customRenderAnimationCount + ")", customRender);
    }
  ...
  onGestureElementMoved: function onGestureElementMoved(/*e*/) {
    ...
    if (element === this.canvas.element) {
      this.onCanvasElementMoved();
    }

    if (dom.hasClass(element, this.config.itemClassName)) {
      this.onRenderElementMoved(e);
    }
    */
  ...
  onRenderElementMoved: function onRenderElementMoved() {
    /*
    var element = e.data;
    var itemId = dom.attr(element, "data-item-id");
    var renderId = dom.attr(element, "data-render-id");
    var render = this.renders[itemId][renderId];

    if (!render) {
      return;
    }

    // (would like to debounce this, but that would required a memoized debounce per render argument)
    this.loadOrUnloadRender(render);
    */
    this.debouncedLoadOrUnloadRenders();
  }
