...
_.extend(Viewport.prototype, binder, hasEmitter, /** @lends Viewport.prototype */ {
  ...
  configureRenderGestures: function(render) {
    if (render.isErasing) {
  ...
  addRenderGestureHandler: function(render) {
    var gestureHandlerGroupId = render.gestureHandlerGroupId;

    if (!gestureHandlerGroupId) {
      // No group id, don't add gesture handler
      return;
    }

    if (!this.gestureHandlerGroups[gestureHandlerGroupId]) {
      console.log("Viewport#addRenderGestureHandler: creating gesture group: " + gestureHandlerGroupId);
      this.gestureHandlerGroups[gestureHandlerGroupId] = new GestureHandlerGroup({
        config: this.config,
    ...
    if (gestureHandlerGroup.hasGestureHandlerForElement(render.element)) {
      // Already has element in a group
      return;
    }
    ...
    console.log("Viewport#addRenderGestureHandler: adding gesture handler", render);
    gestureHandlerGroup.addGestureHandler(gestureHandler);
  },

  removeRenderGestureHandler: function(render) {
    var gestureHandlerGroupId = render.gestureHandlerGroupId;
    ...
    console.log("Viewport#removeRenderGestureHandler: remove gesture handler", render);
