...
_.extend(Viewport.prototype, binder, hasEmitter, /** @lends Viewport.prototype */ {
  ...
  drawItems: function(options) {
    ...
    // If there is nothing to draw, emit an event to indicate we're done
    if (items.length === 0) {
  ...
  drawCustomRenders: function() {
    ...
    _.each(layoutCustomRenders, function(customRender, index) {
      customRender.id = customRender.id || ("" + index);
   ...
   * @return {undefined}
   */
  createCustomRenderElement: function(id) {
    var element = dom.create("div");
    ...
    if (_.isString(id) && id.length > 0) {
      dom.setAttr(element, "data-custom-render-id", id);
    }
    return element;
  },
  ...
  onItemCollectionItemAdding: function() {
    if (this.config.debugDrawing) {
      console.info("Viewport#onItemCollectionItemAdding: erasing custom renders");
    }
  ...
  onItemCollectionItemAdded: function() {
    /*
    if (this.config.debugDrawing) {
      console.info("Viewport#onItemCollectionItemRemoved: erasing custom renders");
    }
  ...
  onItemCollectionItemRemoving: function() {
    if (this.config.debugDrawing) {
      console.info("Viewport#onItemCollectionItemRemoving: erasing custom renders");
    }
  ...
  /**
   * Called when an {@link Item} is removed from the {@link ItemCollection}.  Triggers an erase of the {@link Item}.
   *
   * @param e
   * @return {undefined}
   */
  ...
  /**
   * Called when an {@link Item} is changed.  Triggers a redraw/reload on the {@link Item}.
   *
   * @return {undefined}
   */
  ...
  /**
   * Called when a {@link Item} is changed.  Triggers a redraw/reload of the {@link Item}
   *
   * @return {undefined}
   */
  ...
  /**
   * Called when the {@link ItemCollection} is clearing (before).
   *
   * @return {undefined}
   */
  onItemCollectionClearing: function() {
    if (this.config.debugDrawing) {
      console.info("Viewport#onItemCollectionClearing: erasing custom renders");
    }
    this.eraseCustomRenders();
  },

  /**
   * Called when the {@link ItemCollection} is indexing (before).
   *
   * @return {undefined}
   */
  onItemCollectionIndexing: function() {
    if (this.config.debugDrawing) {
  ...
  /**
   * Called when the {@link ItemCollection} is indexed (after).
   *
   * @return {undefined}
   */
  onItemCollectionIndexed: function(e) {
    var stats = e.data;
    if (this.config.debugDrawing) {
      console.info("Viewport#onItemCollectionIndexed: erasing custom renders");
    }

    // If no indices were changed, there will be no re-draw operation, so re-draw the custom renders now
    ...
    if (self.rendersDrawingCount === 0) {
      setTimeout(function() {
        ...
        setTimeout(function() {
          ...
          setTimeout(function() {
            if (self.config.debugDrawing) {
              console.info("Viewport#onRenderAnimationComplete: drawing custom renders");
            }
