...
_.extend(Viewport.prototype, binder, /** @lends Viewport.prototype */ {
  ...
  drawRenders: function(item, renders) {
    ...
    this._rendersDrawing += (renderIdsToMerge.length + renderIdsToRemove.length + renderIdsToAdd.length);
  ...
  */
  ...
  onAnimationComplete: function(render) {
    ...
    console.log("_rendersDrawing", this._rendersDrawing);
    this._rendersDrawing--;
    if (this._rendersDrawing === 0) {
      console.log("done drawing renders", this._rendersDrawing);
      services.emitter.emit(DecksEvent("viewport:all:renders:drawn", this));
    }
