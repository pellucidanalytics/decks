...
_.extend(Viewport.prototype, binder, hasEmitter, /** @lends Viewport.prototype */ {
   ...
   *
   * 2. Initiate the async draw process for each render.
   *
  ...
  getRenders: function(item) {
    ...
    if (!item) {
      return _(this.renders) // object with that have item ids as keys
  ...
  drawRender: function(render) {
    ...
    raf(function() {
      // This event is handled by the Canvas (among other things), to make sure the element is in the DOM
      // before the animation starts.
      // Do this in one animation frame, and then start the animation in the next frame
  ...
  drawRenders: function(item, renders) {
    ...
    _.each(renderIdsToMerge, function(renderId) {
      ...
      // TODO: this could probably be _.extend
  ...
  setDefaultCustomRenderAnimateOptions: function(customRender) {
    ...
    customRender.animateOptions.complete = function(elements) {
      customRender.animationCompleteData = {
        elements: elements
      };
  ...
  createCustomRenderElement: function(customRender) {
    validate(customRender, "customRender", { isRequired: true });
    ...
    dom.setStyle(element, "top", 0);
    dom.setStyle(element, "left", 0);
    dom.setAttr(element, "data-custom-render-id", customRender.id);
  ...
  initializeCustomRender: function(customRender) {
    validate(customRender, "customRender", { isRequired: true });
    this.layout.initializeCustomRender(customRender, this.layoutMethodOptions);
  },
  ...
  loadRender: function(render) {
    ...
    // TODO: should there be a requestAnimationFrame here?
  ...
  shouldLoadRender: function(render) {
    ...
    // TODO: not sure what's the best ordering/combination of these checks
  ...
  panToItem: function(item, renderIdOrIndex) {
    ...
    if (!render) {
      throw new Error("Viewport#panToItem: No render found for item " + item.id + " with render id or index " + renderIdOrIndex);
    }

    if (!render.element) {
      throw new Error("Viewport#panToItem: No render element found for item " + item.id + " with render id or index " + renderIdOrIndex);
    }

    var panOffsets = this.layout.getPanToItemOffsets(item, render, this.layoutMethodOptions);

    this.canvas.panToElement(render.element, panOffsets.x, panOffsets.y);
  },
  ...
  drawCustomRenders: function() {
    var layoutCustomRenders = this.layout.getCustomRenders(this.getCustomRenders(), this.layoutMethodOptions) || [];
    ...
    _.each(layoutCustomRenders, function(customRender, index) {
      customRender.id = "" + index;
      customRenders[customRender.id] = customRender;
    }, this);

    var customRenderIds = _.keys(customRenders);
    var previousCustomRenders = this.getCustomRenders();
    var previousCustomRenderIds = _.keys(previousCustomRenders);
    var customRenderIdsToMerge = _.intersection(customRenderIds, previousCustomRenderIds);
    var customRenderIdsToAdd = _.difference(customRenderIds, customRenderIdsToMerge);
    var customRenderIdsToRemove = _.difference(previousCustomRenderIds, customRenderIdsToMerge);

    _.each(customRenderIdsToMerge, function(id) {
      var mergedCustomRender  = _.merge({}, previousCustomRenders[id], customRenders[id]);
      this.drawCustomRender(mergedCustomRender);
    }, this);

    _.each(customRenderIdsToAdd, function(id) {
      var customRender = customRenders[id];
      customRender.element = this.createCustomRenderElement();
      this.initializeCustomRender(customRender);
      this.drawCustomRender(customRender);
    }, this);

    _.each(customRenderIdsToRemove, function(id) {
      var customRender = previousCustomRenders[id];
  ...
  onDeckReady: function() {
    ...
    // Flag the items as needing to be loaded, because they haven't been loaded before
  ...
  onDeckLayoutSet: function(e) {
    ...
    // Flag the items as needing to be re-loaded because different layouts may require different
    // loading logic/UI representations for each render.
    this.drawItems({ isLoadNeeded: true });
  },
  ...
  onDeckDraw: function() {
    ...
    // Flag the items as needing to be re-loaded, because this event is from an explicit call to deck.draw()
    // by the app - which is requesting a re-draw/re-load.
    this.drawItems({ isLoadNeeded: true });
  },
  ...
  onFrameBoundsSet: function() {
    ...
    // Don't need to re-load the items if just re-drawing due to a frame size/location change
    this.drawItems();
  },
  ...
  onItemChanged: function(e) {
    ...
    // Flag for re-load here, because the data of the item has changed, so it might need to be re-loaded
  ...
  onItemIndexChanged: function(e) {
    ...
    // TODO: not sure if load needed on item index changed
    // Maybe so if an index changes, and the layout renders things differently based on index (?)
    this.drawItem(item, { isLoadNeeded: true });
  },
  ...
  onRenderAnimationComplete: function(render) {
    ...
    if (render.isErasing) {
      // Erase animation completed - if render is flagged for removal, remove it now
      self.removeRender(render);

      // If the render's item is flagged for removal, and all renders are now removed, remove the item
      if (render.item.isErasing && !self.hasRenders(render.item)) {
        self.removeItem(render.item);
      }
    } else {
      // Draw animation completed - save the render in the internal data structure
    ...
    if (self.rendersDrawingCount === 0) {
      // If the current draw cycle is complete:
      // 1. notify anyone that cares
      // 2. load or unload renders
      // 3. do a draw cycle on custom renders
      setTimeout(function() {
  ...
  onCustomRenderAnimationComplete: function(customRender) {
    ...
    if (customRender.isErasing) {
      self.removeCustomRender(customRender);
    } else {
    ...
    if (self.customRendersDrawingCount === 0) {
      setTimeout(function() {
