...
_.extend(Viewport.prototype, eventBinder, /** @lends Viewport.prototype */ {
  ...
  drawItem: function(item, options) {
    ...
    var layoutRenders = this.layout.getRenders(item);
    if (!_.isArray(layoutRenders)) {
      layoutRenders = [layoutRenders];
    }

    //console.log("drawItem", item, layoutRenders);
  ...
  createRenderElement: function(item, render) {
    ...
    element.setAttribute("data-render-id", render.id);
    return element;
  },
  ...
  _itemCollectionEvents: {
    ...
    "item:collection:item:index:changed": "_onItemIndexChanged",
  ...
  _onAnimationBegin: function(render) {
    ...
    this.emit("viewport:animation:begin", render);
  },
  ...
  _onAnimationComplete: function(render) {
    ...
    this.emit("viewport:animation:complete", render);
  },
