...
_.extend(Viewport.prototype, binder, hasEmitter, /** @lends Viewport.prototype */ {
  ...
  drawRender: function(render) {
    ...
    // some configs (need to find a better place)
    var itemsPerSlot = 15,
        maxSlotIndexForAnimations = 2, /* animations are only applied for itemsPerSlot * (maxSlotIndexForAnimations + 1) items */
        slotRenderDelay = 100;


    // reset duration to (max) 200ms (should happen in CDP)
    render.animateOptions.duration = render.animateOptions.duration > 200 ? 200 : render.animateOptions.duration;

    // reset delay to 0 (render and animation are staggered using timer) (should happen in CDP)
    render.animateOptions.delay = 0;

    var renderSlotIndex = parseInt(self.rendersDrawingCount / itemsPerSlot, 10);
    if (renderSlotIndex > maxSlotIndexForAnimations ||
  ...
  onAnimationComplete: function(render) {
    ...
    // TODO: not sure if it's better to load renders as each individual animation completes,
    // or just load them all when all animations in the draw cycle complete

    self.rendersDrawingCount--;

    if (self.rendersDrawingCount === 0) {
      // If the current draw cycle is complete, notify anyone that cares
      raf(function() {
