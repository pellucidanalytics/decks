...
_.extend(GestureHandler.prototype, binder, hasEmitter, /** @lends GestureHandler.prototype */ {
  ...
  emitterEvents: {
    ...
    // Scroll
    "gesture:scroll": "onGestureScroll"
  },
  ...
  setOptions: function(options) {
    ...
    if (this.movement.scroll && !_.isElement(this.containerElement)) {
      throw new Error("GestureHandler#setOptions: for options.movement.scroll === true, options.containerElement is required");
    }
    ...
    _.each(options.gestures, function(gestureEmitterOptions, key) {
      if (key === "scroll" && this.movement.scroll) {
        gestureEmitterOptions.enabled = true;
      }
      ...
      _.extend(gestureEmitterOptions, {
        element: element, // Certain events come from the containerElement, rather than the element (e.g. scroll)
        hammer: this.hammer,
  ...
  destroy: function() {
    ...
    // Remove the "touch-action" style (hammer doesn't seem to do this)
    // TODO: the problem is that hammer defaults to touch-action: pan-y, but if the gesture handlers
    // aren't enabled, none of the options are taken into effect, so hammer doesn't change it to pan-x
    // when needed.
    dom.removeStyle(this.element, "touch-action");
  ...
  animateMoveForPanX: function(e, animateOptions, beginOptions, completeOptions) {
    ...
    _.extend(animateOptions, {
      duration: 0,
      queue: false // Don't queue the pan animation, because it's probably just a pixel or two, and needs to be immediate
    });
   ...
   * @return {undefined}
   */
  animateMoveToElement: function(element, animateOptions, beginOptions, completeOptions) {
   ...
   * @return {undefined}
   */
  snapToNearestChildElement: function() {
    if (!this.snapping.toNearestChildElement || !_.isString(this.snapping.childElementSelector)) {
      return;
    }

    console.log("snap nearest");

    var childElements = dom.query(this.snapping.childElementSelector, this.containerElement);
    var nearestChildElement = dom.nearest({ x: 0, y: 0 }, childElements);

    var animateOptions = _.extend({}, this.snapping.animateOptions);
    var beginOptions = {};
    var completeOptions = {
      description: "snap nearest",
      snapToBounds: true,
      snapToNearestChildElement: false,
      clearPositionData: true,
      event: DecksEvent("gesture:snapped:to:nearest:child:element", this, nearestChildElement)
    };
  ...
  onAnimationComplete: function(options) {
    var self = this;
    console.log("complete: " + options.description);
  ...
  onGesturePanEnd: function(e) {
    ...
    if (e.sender.element !== self.element) {
      return;
    }
    ...
    _.defer(function() {
      console.log("pan end (isAnimating: %s)", self.isAnimating());
  ...
  onGestureScroll: _.debounce(function(e) {
    if (e.sender.element !== this.containerElement) {
      return;
    }
