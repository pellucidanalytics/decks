...
var binder = {
  ...
  bindEvents: function(source, eventToMethodMap) {
    _.each(eventToMethodMap, function(methodName, eventName) {
      if (!_.isFunction(this[methodName])) {
        throw new Error("binder#bindEvents: target object does not have a method named " + methodName);
      }

      var eventNames = this.getEventNames(eventName);

      _.each(eventNames, function(eventName) {
        var bindMethodName = this.getBindMethodName(source, eventName);
  ...
  unbindEvents: function(source, eventToMethodMap) {
    _.each(eventToMethodMap, function(methodName, eventName) {
      ...
      _.each(eventNames, function(eventName) {
        var unbindMethodName = this.getUnbindMethodName(source, eventName);
        ...
        if (eventName === "*") {
          ...
          // IE8 prefixes event names with "on"
          if (unbindMethodName === "detachEvent") {
            eventName = "on" + eventName;
          }
   ...
   */
  getBindMethodName: function(source, eventName) {
    if (eventName === "*") {
      if (_.isFunction(source.onAny)) {
        return "onAny";
      } else {
        throw new Error("binder#getBindMethodName: event source object does not have an onAny method");
      }
    }
    ...
    if (!name) {
      throw new Error("binder#getBindMethodName: event source object does not have an event binding method");
    }
   ...
   */
  getUnbindMethodName: function(source, eventName) {
    if (eventName === "*") {
      if (_.isFunction(source.offAny)) {
        return "offAny";
      } else {
        throw new Error("binder#getUnbindMethodName: event source does not have an offAny method");
      }
    }
    ...
    if (!name) {
      throw new Error("binder#getUnbindMethodName: event source object does not have an event unbinding method");
    }
