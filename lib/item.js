...
_.extend(Item.prototype, hasEmitter, /** @lends Item.prototype */ {
  ...
  setId: function(data) {
    if (_.isString(this.id)) {
      throw new Error("Item#setId: id cannot be changed");
    }
  ...
  setIndex: function(data, options) {
    ...
    if (!options.silent) {
      this.emitter.emit(DecksEvent("item:index:changed", this, this.index));
    }
  },
  ...
  get: function(key) {
    if (!_.isString(key)) {
      throw new Error("Item#get: key is required and must be a string");
    }
  ...
  set: function(key, value, options) {
    if (!_.isString(key)) {
      throw new Error("Item#set: key is required and must be a string");
    }
    ...
    if (!options.silent) {
      this.emitter.emit(DecksEvent("item:changed", this, { key: key, value: value, oldValue: oldValue }));
    }
  },
  ...
  setData: function(data, options) {
    ...
    if (!options.silent) {
      this.emitter.emit(DecksEvent("item:changed", this, { oldData: oldData, data: data }));
    }
  },
  ...
  clear: function(options) {
    ...
    if (!options.silent) {
      this.emitter.emit("item:cleared", this);
    }
  }
});
