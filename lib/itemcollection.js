...
_.extend(ItemCollection.prototype, binder, hasEmitter, /** @lends ItemCollection.prototype */ {
  ...
  addItems: function(items, options) {
    ...
    _.each(items, function(item) {
      this.addItem(item, { noIndex: true }); // don't re-index the collection for each item, do it once at the end
    }, this);
   ...
   * the filter/sortBy/reversed version of the {@link ItemCollection} items.
   *
   ...
   * @return {undefined}
   */
  index: function(reason) {
    validate(reason, "reason", { isRequired: true });

    this.emit(DecksEvent("item:collection:indexing", this, reason));
